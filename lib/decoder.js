    ...
    class Decoder {
        ...
        getSamples(isInterleaved) {
            if (this.metadata) {
                ...
                if (isInterleaved) {
                    return (0, wav_utils_1.interleave)(this.data, channels, this.metadata.bitsPerSample);
                }
                const data = new Array(channels);
                for (let i = channels - 1; i >= 0; --i) {
                    const chData = this.mapData(d => d[i]);
                    data[i] = (0, data_utils_1.mergeBuffers)(chData, (0, data_utils_1.getLength)(chData));
                }
                return data;
            }
        ...
        _canReadChunk() {
            return (0, data_utils_1.getLength)(this._inputCache) >= this._min_data_decode_threshold;
        }
        _resetInputCache() {
            this._decodingChunkPaused = true;
            this._currentInputCacheOffset = -1;
